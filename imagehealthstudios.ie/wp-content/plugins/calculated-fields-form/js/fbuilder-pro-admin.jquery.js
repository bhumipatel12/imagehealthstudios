	...
	$.fbuilder[ 'checkCopyToUser' ] = function()
	{
		var copy_to_user = $( '[name="cu_enable_copy_to_user"]' );
		if( copy_to_user.length && copy_to_user.val() == 'true' )
		{
			var emails_list = $( '[id="cu_user_email_field"]' );
			if(
				emails_list.length &&
				emails_list.find( 'option' ).length &&
				emails_list.find( 'option:selected' ).length == 0
			)
			{
				alert( 'If the attribute: "Send confirmation/thank you message to user?" is set to "Yes", must be selected at least an email field from the list: "Email field on the form".' );
				return false;
			}
		}
		return true;
	...
	{
		if( !$.fbuilder[ 'checkCopyToUser' ]() ) return;
		...
		$.fbuilder[ 'defineGeneralEvents' ] = function()
			{
				...
				{
					theForm.request_cost = $(this).val();
					$.fbuilder.reloadItems({'silent':1});
				});
		...
		$.fbuilder[ 'reloadItems' ] = function( args )
			{
				...
				if(typeof args['silent'] === 'undefined')
				{
					...
					else
					{
						...
						$('#request_cost').html(cost_str).change();
	...
	$( '[name="cpformconf"]' ).on( 'submit', function(){
		return $.fbuilder[ 'checkCopyToUser' ]();
	} );