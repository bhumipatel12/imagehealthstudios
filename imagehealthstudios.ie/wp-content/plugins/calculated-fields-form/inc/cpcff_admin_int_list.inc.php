...
<script type="text/javascript">
 ...
 function cp_updateConfig()
 {
    if (confirm('<?php _e( 'Are you sure you want to update these settings?', 'calculated-fields-form' ); ?>'))
    {
		...
		document.location = 'admin.php?page=cp_calculated_fields_form&ecr='+ecr+'&ac=st&scr='+scr+'&chs='+chs+'&jsc='+jsc+'&optm='+optm+'&cdm='+cdm+'&fmc='+fmc+'&ehr='+ehr+'&em='+em+'&es='+es+'&df='+df+'&amp='+amp+'&typc='+typc+'&nc='+nc+'&hp='+encodeURIComponent( hp )+'&r='+Math.random()+'&_cpcff_nonce=<?php echo wp_create_nonce( 'cff-update-general-settings' ); ?>';
    }
 }
            ...
            <div class="inside">
                <form name="updatesettings">
                    <div style="border:1px solid #DADADA; padding:10px;">
                        ...
                        <input type="checkbox" name="ccencodingemail" id="ccencodingemail" <?php echo ( get_option( 'CP_CALCULATEDFIELDSF_ENCODING_EMAIL', false ) ) ? 'CHECKED' : ''; ?> /><em><?php _e('* Encodes the notification emails as ISO-8859-2 and base64.', 'calculated-fields-form' ); ?></em>
                    </div>
                    <br />
