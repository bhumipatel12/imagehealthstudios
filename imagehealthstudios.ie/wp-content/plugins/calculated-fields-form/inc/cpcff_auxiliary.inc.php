...
if(!class_exists('CPCFF_AUXILIARY'))
{
	...
	class CPCFF_AUXILIARY
	{
        ...
        public static function replace_params_into_url($url, $params = [])
        {
            try
            {
                $parts = parse_url($url);
                    ...
                    if(!empty($query_params))
                    {
                        foreach($query_params as $param_name => $param_value)
                        {
                            if(preg_match_all('/<%([^%]+)%>/i', $param_value, $matches))
                            {
                                ...
                                {
                                    if(isset($params[$fieldname.'_urls'])) $replacement = $params[$fieldname.'_url'];
                                    elseif(isset($params[$fieldname])) $replacement = $params[$fieldname];
                                    else $replacement = '';
		...
		public static function encrypt($plain)
		{
			...
			$encrypted = function_exists('openssl_encrypt') ? openssl_encrypt($plain, "AES-256-CBC", $key, 0, $iv) : false;
		...
		public static function decrypt($encrypted)
		{
			if($encrypted)
			{
				...
				$ivlen = openssl_cipher_iv_length("AES-256-CBC");
				$iv = substr(md5($key), 0, $ivlen);
				$plain = function_exists('openssl_decrypt') ? openssl_decrypt($encrypted, "AES-256-CBC", $key, 0, $iv) : false;
			}
			return !empty($plain) ? $plain : false;
		} // End decrypt
		...
		public static function parsing_fields_on_text($fields, $params, $text, $summary, $format, $postid)
		{
										...
										switch($in_tag)
										{
											...
											case '<img>':
												foreach($value as $_i => $_url)
													$value[$_i] = (@is_array(getimagesize($_url))) ? '<img src="'.esc_attr($_url).'">' : $_url;
											break;
											case 'a':
											case '<a>':
												foreach($value as $_i => $_url)
			...
			return array( 'text' => apply_filters('cpcff_custom_tags', $text, $postid), 'files' => $attachments );
		...
		{
			$tags_arr = array();
			$new_tags = apply_filters('cpcff_additional_tags',array());
			$additional_tags = (!empty($new_tags)) ? "|".implode("|", $new_tags) : "";

			if(
				preg_match_all("/<%(info|fieldname\d+|fieldname\d+_label|fieldname\d+_block|fieldname\d+_endblock|fieldname\d+_shortlabel|fieldname\d+_value|fieldname\d+_url|fieldname\d+_urls|coupon|couponcode|itemnumber|formid|subscription_id|transaction_id|final_price|payment_option|ipaddress|from_page|thank_you_page|currentdate_mmddyyyy|currentdate_ddmmyyyy|currenttime|submissiondate_mmddyyyy|submissiondate_ddmmyyyy|submissiontime|payment_status".$additional_tags.")\b(?:(?!%>).)*%>/i",
					$text,
					$matches
				)
			)
