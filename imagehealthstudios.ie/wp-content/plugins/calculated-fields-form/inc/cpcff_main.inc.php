...
if(!class_exists('CPCFF_MAIN'))
{
	...
	class CPCFF_MAIN
	{
		...
		{
			// Settings page
			add_options_page('Calculated Fields Form Options', 'Calculated Fields Form (Platinum)', 'manage_options', 'cp_calculated_fields_form', array($this, 'admin_pages') );

			// Menu option
			add_menu_page( 'Calculated Fields Form Options', 'Calculated Fields Form (Platinum)', 'manage_options', 'cp_calculated_fields_form', array($this, 'admin_pages') );
		...
		public function admin_pages()
		{
			...
			else
			{
				...
				if (isset($_GET["page"]) &&$_GET["page"] == 'cp_calculated_fields_form_sub2')
				{
					if(@wp_redirect('https://cff.dwbooster.com/documentation')) exit;
				}
				elseif (isset($_GET["page"]) && $_GET["page"] == 'cp_calculated_fields_form_sub5')
				{
		...
		public function admin_resources( $hook )
		{
			if ( isset($_GET['page']) )
			{
				...
				elseif('cp_calculated_fields_form_sub2' == $_GET["page"])
				{
					...
					wp_localize_script(
						...
						'cpcff_redirect',
						array('url' => 'https://cff.dwbooster.com/documentation')
					);
				}
			}
		...
		public function form_result_shortcode($atts, $content = "", $id = 0)
		{
			...
			return apply_filters( 'cpcff_summary', $output );
		...
		public function form_result_list_shortcode($atts, $content = "")
		{
			...
			if(
				isset($atts['formid']) &&
				($formid = trim($atts['formid'])) != ''
			)
			{
				 $formid = @intval($formid);
				if(!empty($formid)) $cond .= $wpdb->prepare(" AND formid=%d", $formid);
			}
                        ...
                        '<td style="width:100px;white-stace:nowrap;">
                            ...
                            <div class="cff_submission_details" style="display:none;">
                                ...
                                <div class="cff_submission_details_content">'.
                                    $this->form_result_shortcode($atts, !empty($content) ? $content : '<%INFO%>', $submission->id).
                                '</div>
                            </div>
                        </td>' : '').
                        '</tr>';
