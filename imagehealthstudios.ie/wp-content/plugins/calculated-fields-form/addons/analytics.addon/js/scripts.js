		...
		window[ 'cpcff_analytics_submit_event' ] = function( form_element, form_id )
			{
				...
				ga(
					form_list[ form_id ][ 'tracker' ]+'.send',
					'event',
					'form',
					'submit',
					'Form '+form_id+' - Submit Form',
					{
						'hitCallback': function()
						{
							form_obj.data( 'sent_analytics_submit', 1 )
									.submit();
						}
					}
				);
						...
						if( form_seq.length )
						{
							...
							if( form_id_obj.length )
							{
								...
								ga(
									...
									'exception',
									{
										'exDescription': 'Form '+form_id+' - Invalid CAPTCHA Code',
										'exFatal': false
									}
								);
							}
		...
		for( var form_id in form_list )
		{
			...
			$( '[name="cp_calculatedfieldsf_id"][value="'+form_id+'"]').each(
				function()
				{
					...
					// Create tracker
					ga( 'create', $.trim( form_settings[ 'property' ] ), 'auto', form_settings[ 'tracker' ] );

					// Load form event
					if( checkEvent( 'load', form_settings ) )
					{
						ga(
							form_settings[ 'tracker' ]+'.send',
							'event',
							'form',
							'load',
							'Loaded Form '+form_id
                                ...
                                if( checkEvent( 'next_page', form_settings ) )
                                {
                                    if( form_settings[ 'page_bk' ] < current_page )
                                    {
                                        ga(
                                            ...
                                            'form',
                                            'next page',
                                            'Form '+form_id+' - Page '+current_page );
                                    }
                                }
